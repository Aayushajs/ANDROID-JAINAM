...
const CheckoutPage = ({ navigation }) => {
  ...
  // Set navigation bar color for Android soft keys background
  React.useEffect(() => {
    const navColor = isDark ? '#000' : '#fff';
    if (Platform.OS === 'android') {
      if (StatusBar.setBackgroundColor) {
        StatusBar.setBackgroundColor(navColor, true);
      }
      if (SystemUI.setBackgroundColorAsync) {
        SystemUI.setBackgroundColorAsync(navColor);
      }
    }
  }, [isDark]);
  ...
  const calculateItemDiscount = (item) => {
    const originalPrice =
      item.price ||
      item.finalPrice * (100 / (100 - (item.discountPercent || 0)));
    return (
      (originalPrice - (item.finalPrice || item.price)) * (item.quantity || 1)
    );
  };
  ...
  const calculateFinalTotal = () =>
    calculateTotal() + calculateTax() + calculateShipping();
  ...
  const renderAddressItem = ({ item, index }) => (
    <TouchableOpacity
      style={[
        styles.addressItem,
        {
          ...
          borderColor:
            selectedAddress === index ? "#10B981" : isDark ? "#333" : "#E5E7EB",
        },
      ]}
    ...
    >
      ...
      <Text
        style={[styles.addressText, { color: isDark ? "#E5E7EB" : "#4B5563" }]}
      >
        {item.address}
      </Text>
      <Text
        style={[styles.contactText, { color: isDark ? "#9CA3AF" : "#6B7280" }]}
      >
      ...
      <View style={styles.productInfo}>
        ...
        <View style={styles.priceContainer}>
          ...
          <TouchableOpacity
            style={[
              styles.couponBtn,
              { borderWidth: 1, borderColor: isDark ? "#fff" : "#000" },
            ]}
            onPress={() => handleSinglePayment(item)}
          >
            <Text
              style={[
        ...
        >
          ...
          >
            {expandedItems[item.id] ? "Hide Details" : "More Details"}
          </Text>
          <Icon
        ...
        {expandedItems[item.id] && (
          ...
          >
            ...
            <View style={styles.detailRow}>
              ...
              <Text style={[styles.discountValueLarge, { color: "#10B981" }]}>
            ...
            >
              <Text
                style={[
                  styles.finalLabelLarge,
                  { color: isDark ? "#F9FAFB" : "#111827" },
                ]}
              >
                Item Total
              </Text>
              <Text style={styles.finalValueLarge}>
                ₹{calculateItemTotal(item).toFixed(2)}
              </Text>
            </View>
          </View>
        ...
        <View style={styles.section}>
          ...
          {cart.length === 0 ? (
            ...
            <FlatList
              ...
              scrollEnabled={false}
              keyExtractor={(item) =>
                item.id + "-" + (item.selectedUnit || "default")
              }
